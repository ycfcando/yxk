...
export default {
  ...
  computed: {
    ...
    ccDate() {
      if (this.$store.state.ccList) {
        return this.$store.state.ccList;
      } else {
        return [];
      }
    },
  ...
  mounted() {
    this.$store.dispatch("getTravelCc", {
      daysId: "",
      startDate: "",
      sitecode: 1,
      page: this.count,
  ...
  methods: {
    onLoad() {
      this.$nextTick(() => {
        ...
        if (this.$store.state.ccList >= 60) {
          this.finished = true;
        }
      });
    },
    changeType(val) {
      this.$store.state.ccList = [];
      this.count = 1;
      this.mdd = val;
      this.$store.dispatch("getTravelCc", {
        daysId: "",
